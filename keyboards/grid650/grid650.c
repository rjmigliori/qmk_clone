...
void matrix_scan_kb(void) {
    ...
    #ifdef BATTERY_LEVEL
    static uint8_t bat_counter = BATTERY_POLL;
    ...
    if (counter > BASIC_CNT) {
        ...
        #ifdef BATTERY_LEVEL        
        #ifdef ENABLE_SLEEP
        if(sleep_flag == 0) bat_counter++;
        #else
        bat_counter++;
        #endif
        #endif
        #ifdef ENABLE_SLEEP
        if(sleep_flag==0) sleep_counter++;
        ...
        #ifdef BATTERY_LEVEL
        #ifdef ENABLE_SLEEP
        if(bat_counter >= BATTERY_POLL && sleep_flag == 0){
        #else
        ...
        // if(bat_counter >= 1){
            ...
            //     adafruit_ble_send_batlevel(bat_level_sum);
            // }

            xprintf("send bat level to BT\n");
            batt_level = battery_level();
            #ifdef BLE_BATTERY_REPORT
            adafruit_ble_send_batlevel(batt_level);
            #endif
        }
        #endif
    ...
    // if (has_anykey(keyboard_report)) {
        ...
        sleep_flag = 1;
        // PORTB |= (1 << 7); //turn on led orange
        // PORTB &= ~(1 << 5);//turn off led red
